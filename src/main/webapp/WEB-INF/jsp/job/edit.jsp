...
<form action="" method="post">
    ...
    <option value="1" <c:if test="${job.jobCategory == '1'}"> <c:out value="selected="></c:out> </c:if> >PHP</option>
    <option value="2" <c:if test="${job.jobCategory == '2'}"> <c:out value="selected="></c:out> </c:if> >SHELL</option>
    <option value="3" <c:if test="${job.jobCategory == '3'}"> <c:out value="selected="></c:out> </c:if> >JAVA</option>
    </select></td></tr>
     <tr><td align="right">执行命令:</td><td><input name="parameters" type="text" value="<c:out value="${job.parameters}"/>"/></td></tr>
